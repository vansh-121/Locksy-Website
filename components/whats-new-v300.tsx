...
export default function WhatsNewV300() {
  const [activeTab, setActiveTab] = useState<string>("privacy-reports")
          ...
          <div className="lg:col-span-5 space-y-4">
            {V300_FEATURES.map((feature) => (
              <button
                ...
                className={`w-full text-left p-6 rounded-3xl border transition-all duration-500 relative overflow-hidden group outline-none ${activeTab === feature.id
              ...
              >
                ...
                <div className={`absolute left-0 top-0 bottom-0 w-1 transition-all duration-500 ${activeTab === feature.id ? "bg-gradient-to-b from-violet-500 to-fuchsia-500" : "bg-transparent group-hover:bg-border"
                  }`} />

                <div className="flex gap-5 items-start relative z-10 ml-2">
                  <div className={`flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${activeTab === feature.id
                  ...
                  <div className="flex-1">
                    <span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase mb-3 transition-colors duration-300 ${activeTab === feature.id
                        ? "bg-violet-500/10 text-violet-600 dark:text-violet-400"
                        : "bg-muted text-muted-foreground"
                      }`}>
                      {feature.badge}
                    </span>
                    <h3 className={`font-bold text-lg md:text-xl mb-2 transition-colors duration-300 ${activeTab === feature.id ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"
                      }`}>
                      {feature.title}
                    </h3>
                    <p className={`text-sm leading-relaxed transition-all duration-300 ${activeTab === feature.id ? "text-muted-foreground" : "text-muted-foreground/70 line-clamp-2"
                      }`}>
                      {feature.description}
                    </p>
                  </div>
                      ...
                      <div className="bg-card border border-border/50 p-1.5 rounded-2xl flex gap-1 mb-6 shadow-sm">
                        {[
                          ...
                          <button
                            ...
                            className={`flex-1 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all duration-300 ${mockActiveNav === tab.id
                          ...
                          <div className="space-y-4 animate-in slide-in-from-right-4 fade-in duration-300 p-2">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              {["5 Mins", "15 Mins", "Custom"].map((t, idx) => (
                                <div key={idx} className={`p-3 rounded-xl border text-center text-xs font-bold transition-all cursor-pointer ${idx === 2 ? "border-violet-500 bg-violet-500/10 text-violet-600 dark:text-violet-400 ring-2 ring-violet-500/20" : "border-border bg-background text-muted-foreground hover:border-foreground/30"
                                  }`}>
                                  {t}
                                </div>
                              ))}
                  ...
                  {activeTab === "intruder-capture" && (
                    <div className="absolute inset-0 p-8 flex flex-col animate-in fade-in zoom-in-95 duration-500">
                      ...
                      <div className="relative flex-1 rounded-2xl border-2 border-border overflow-hidden bg-black flex items-center justify-center shadow-inner group">
                        ...
                        {/* Flash overlay */}
                        <div className={`absolute inset-0 bg-white transition-opacity duration-150 z-30 pointer-events-none ${showFlash ? "opacity-100" : "opacity-0"
                          }`} />
                      </div>
                    </div>
                  )}
